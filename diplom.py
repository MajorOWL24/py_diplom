...
url = "https://api.vk.com/method/photos.get?owner_id=" + vk_id + "&album_id=profile&extended=1&access_token=" + vk_token + "&v=5.131"
r = requests.get(url)
...
for likes in data:
  ...
  for photo in data[likes]:
    ...
    r = requests.put(url, headers=yandex_headers)

    filedata = requests.get(photo["url"])
    url = "https://cloud-api.yandex.net/v1/disk/resources/upload?path=" + folder + "/" + name + "&overwrite=true"
    r = requests.get(url, headers=yandex_headers).json()
    if "href" in r:
      r = requests.put(r["href"], data=filedata.content, headers=yandex_headers)
